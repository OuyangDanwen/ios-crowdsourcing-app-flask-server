...
@Component({
  template: `
  <div style="text-align: center;" id="action-btn"><button class="btn btn-warning btn-sm action-property" (click)="launchEditModal($event)" title="Edit Resource Name"><i class="ion-edit"></i>&nbsp;Edit</button>
  <button class="btn btn-danger btn-sm action-property" (click)="launchDeleteModal($event)" title="Delete Resource"><i class="ion-trash-a"></i>&nbsp;Delete</button></div>
   `,
})
